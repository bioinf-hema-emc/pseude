...
#' @param agg_table Look-up table that links cells to aggregation groups
#' @param agg_group String with column in agg_table that contains the grouping
...
#'
#' @export
scaggregate <- function(sc_counts,
                        agg_table,
                        agg_group = "sample_name") {
  ...
  return(t.Mat(agg.Mat(t.Mat(sc_counts), groupings = agg_table[, agg_group])))
}
